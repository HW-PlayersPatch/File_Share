...
setLevelOfDifficulty( 0 ) -- 0 = easy, 1 = standard, 2 = hard ( 0 is default )
--
-- INITIALIZATION
--
...
--~ 	
	...
	sg_maxNumCollectors = 0 -- based on unitcap and number of RUs on map
...
--~ 	local timedelay = 80 -- based on map size and other factors
...
function Override_SubSystemDemand()
	...
	-- use the AIs default demand rules for subsystems
	CpuBuildSS_DefaultSubSystemDemandRules()
	-- OR
	-- only demand for these 3 subs - and no others
	SubSystemDemandSet( FIGHTERPRODUCTION, 10)
	SubSystemDemandSet( CORVETTEPRODUCTION, 10)
	--SubSystemDemandSet( FRIGATEPRODUCTION, 10)


	-- make one minor adjustment so the AI is less likely to build the research subsystem
