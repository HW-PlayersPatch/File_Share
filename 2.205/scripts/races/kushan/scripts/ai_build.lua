...
function DetermineSpecialDemand_Kushan()
		...
		ShipDemandSet( KUS_RESEARCHSHIP, 20)	

		local numRes0 = NumSquadrons( KUS_RESEARCHSHIP ) 
		if(numRes0 == 1 ) then
		ShipDemandSet( KUS_RESEARCHSHIP, 0)
		ShipDemandSet( KUS_RESEARCHSHIP_1, 11)	
		end
		local numRes1 = NumSquadrons( KUS_RESEARCHSHIP_1 ) 
		if(numRes1 == 1 ) and (GetRU() > 750) then
		ShipDemandSet( KUS_RESEARCHSHIP_1, 0)
		ShipDemandSet( KUS_RESEARCHSHIP_2, 11)
--		ShipDemandSet( KUS_LIGHTCORVETTE, 0.5)
		end
		local numRes2 = NumSquadrons( KUS_RESEARCHSHIP_2 ) 
		if(numRes2 == 1 ) and (GetRU() > 750) then
		ShipDemandSet( KUS_RESEARCHSHIP_2, 0)	
		ShipDemandSet( KUS_RESEARCHSHIP_3, 11)	
		end
		local numRes3 = NumSquadrons( KUS_RESEARCHSHIP_3 ) 
		if(numRes3 == 1 ) and (GetRU() > 750) then
		ShipDemandSet( KUS_RESEARCHSHIP_3, 0)
		ShipDemandSet( KUS_RESEARCHSHIP_4, 11)	
		end
		local numRes4 = NumSquadrons( KUS_RESEARCHSHIP_4 ) 
		if(numRes4 == 1 ) and (GetRU() > 750) then
		ShipDemandSet( KUS_RESEARCHSHIP_4, 0)
		ShipDemandSet( KUS_RESEARCHSHIP_5, 11)	
		end
		local numRes5 = NumSquadrons( KUS_RESEARCHSHIP_5 ) 
		if(numRes5 == 1 ) and (GetRU() > 750) then
		ShipDemandSet( KUS_RESEARCHSHIP_5, 0)	
		end
		local numRColls = NumSquadrons( KUS_RESOURCECOLLECTOR ) 
		if(numRColls == 22 ) and (GetRU() > 750) then
		ShipDemandSet( KUS_RESEARCHSHIP_5, 0)
		end
	...
	local AssaultFrigateDemand = ShipDemandGet(KUS_ASSAULTFRIGATE)
	...
	if (s_selfTotalValue > 75) and (numEnemyCarriers > 1 ) and (CanBuild( KUS_CARRIER ) == 1 and GetRU() > 2800 )  then
		ShipDemandAddByClass( eBuilder, FrigateDemand + 20.0 )
		Build( KUS_CARRIER )
		end


		kushancarrier = KUS_CARRIER	
		if (numCarriers == 3 ) then
		ShipDemandSet( KUS_CARRIER, 0)
		end


		if (numCarriers > 2 ) and (numCruisers < 3 ) and (numEnemyDestroyers > 2 ) and (GetRU() > 2800 ) then
		ShipDemandAdd( kBattleCruiser, 12.5 )
		Build( KUS_HEAVYCRUISER )
		else
